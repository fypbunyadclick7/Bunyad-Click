...
export default function ManageAdmins() {
  ...
  function getData() {
    fetch(`${process.env.REACT_APP_BACKEND_API_URL}/auth/admin/getAdmins`, {
      method: "GET",
      headers: {
      ...
      .catch((error) => {
        ...
        setUsers([]); // Also fallback in case of error
      });
  }
  ...
  const handleDelete = (id) => {
    fetch(
      `${process.env.REACT_APP_BACKEND_API_URL}/auth/admin/deleteAmdin/${id}`,
      {
        method: "DELETE",
        headers: {
          "api-key": process.env.REACT_APP_BACKEND_API_KEY,
  ...
  const handleSwitchChange = async (active, userId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_API_URL}/auth/admin/toggleAdminStatus/${userId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        toast.success(result.message || "Admin status updated successfully.");
    ...
    <div className="siderow">
      ...
      <MDBModal show={updateModal} setShow={setUpdateModal} tabIndex="-1">
        <MDBModalDialog style={{ borderRadius: 0 }}>
          <MDBModalContent id="card">
            <MDBModalHeader>
              <MDBModalTitle>Update Admin</MDBModalTitle>
              <MDBBtn
                className="btn-close"
                color="none"
